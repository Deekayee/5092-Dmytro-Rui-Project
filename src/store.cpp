...
void Store::updateStockFromShelf(vector<Stock> &stockList, vector<Stock> &shelf)
{
    for (Stock &shelf_item : shelf)
    {
        int id = shelf_item.getStockId();
        Stock *stock_item = Store::findStockById(stockList, id);
...
void Store::changePurchaseFromStock(vector<Stock> &stockList, Stock *olditem, Stock newitem)
{
